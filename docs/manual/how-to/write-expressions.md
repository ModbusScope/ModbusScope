...
This guide shows how to write an expression that calculates or combines data point values.
...
3. Type your expression in the input field. You can reference data points with `${ADDRESS}` and combine them using operators.
...
If the expression or a test input produces an invalid result (such as division by zero or an invalid data point read), the output field shows an error message instead of a value. Correct the expression before applying.
