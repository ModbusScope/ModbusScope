...
1. Stop logging or load a data file so the graph is not actively updating.
2. Place the **left marker** (green): hold `Ctrl` and click the left mouse button on the sample of interest.
3. Place the **right marker** (red): hold `Ctrl` and click the right mouse button on a second sample.

   The **Markers** panel appears on the right side of the window.

   ![Markers panel](<../_static/user_manual/markers.png>)

4. Read the values in the **Markers** panel:
...
5. To move a marker, hold `Ctrl` and click again at a new position using the same mouse button (left for left marker, right for right marker).
...
- [Zoom the graph](../how-to/export-data.md)
