...
3. ModbusScope attempts to detect the field and decimal separators automatically.

   ![CSV import dialog](<../_static/user_manual/import_csv.png>)

4. If the preview shows garbled data, adjust the separator settings manually:
...
5. Set **Timestamp unit** to `ms` or `s` to match the unit in your file. ModbusScope always stores timestamps in milliseconds internally.

6. If the file has header lines or the timestamp is not in the first column, adjust:
