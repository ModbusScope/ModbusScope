...
- **Breaking: adapter protocol version 2.** `adapter.readData` reports QDS quality via an explicit `state`/`flags` model instead of a flat `valid` boolean; `OV`/`SB`/`BL` now arrive usable and labelled instead of being discarded. `adapter.initialize`/`adapter.describe` negotiate a `protocolVersion`. See `docs/plan/quality-descriptors-implementation-plan.md`.
