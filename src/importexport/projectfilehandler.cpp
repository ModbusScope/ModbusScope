...
void ProjectFileHandler::updateProjectSetting(ProjectFileData::ProjectSettings* pProjectSettings)
{
    ...
    {
        applyJsonConnectionSettings(pProjectSettings);
    }
    else
    {
        applyLegacyXmlSettings(pProjectSettings);
    }
...
void ProjectFileHandler::applyJsonConnectionSettings(ProjectFileData::ProjectSettings* pProjectSettings)
{
    ...
    for (int adapterIdx = 0; adapterIdx < adapterList.size(); adapterIdx++)
    {
        ...
        if (adapter.settings.contains(ProjectFileDefinitions::cConnectionsJsonKey))
        {
            ...
            for (const QJsonValue& connVal : std::as_const(connsArray))
            {
                ...
                const quint32 detectedBaud =
                  static_cast<quint32>(connObj[ProjectFileDefinitions::cBaudrateTag].toInt(0));
                if (detectedBaud == QSerialPort::Baud1200 || detectedBaud == QSerialPort::Baud2400 ||
                    detectedBaud == QSerialPort::Baud4800 || detectedBaud == QSerialPort::Baud9600 ||
                    detectedBaud == QSerialPort::Baud19200 || detectedBaud == QSerialPort::Baud38400 ||
                    detectedBaud == QSerialPort::Baud57600 || detectedBaud == QSerialPort::Baud115200)
                {
                    connData->setBaudrate(static_cast<QSerialPort::BaudRate>(detectedBaud));
                }

                const quint32 detectedParity =
                  static_cast<quint32>(connObj[ProjectFileDefinitions::cParityTag].toInt(0));
                if (detectedParity == QSerialPort::NoParity || detectedParity == QSerialPort::EvenParity ||
                    detectedParity == QSerialPort::OddParity)
                {
                    connData->setParity(static_cast<QSerialPort::Parity>(detectedParity));
                }

                const quint32 detectedStopBits =
                  static_cast<quint32>(connObj[ProjectFileDefinitions::cStopBitsTag].toInt(0));
                if (detectedStopBits == QSerialPort::OneStop || detectedStopBits == QSerialPort::OneAndHalfStop ||
                    detectedStopBits == QSerialPort::TwoStop)
                {
                    connData->setStopbits(static_cast<QSerialPort::StopBits>(detectedStopBits));
                }

                const quint32 detectedDataBits =
                  static_cast<quint32>(connObj[ProjectFileDefinitions::cDataBitsTag].toInt(0));
                if (detectedDataBits == QSerialPort::Data5 || detectedDataBits == QSerialPort::Data6 ||
                    detectedDataBits == QSerialPort::Data7 || detectedDataBits == QSerialPort::Data8)
                {
        ...
        for (int idx = 0; idx < matchCount; idx++)
        {
            ...
            deviceData->setInt32LittleEndian(adapterDev[ProjectFileDefinitions::cInt32LittleEndianTag].toBool(true));
        }
    }
 ...
 */
void ProjectFileHandler::applyLegacyXmlSettings(ProjectFileData::ProjectSettings* pProjectSettings)
{
    ...
    for (int idx = 0; idx < connCnt; idx++)
    {
        ...
        if (connectionId < ConnectionTypes::ID_CNT)
        {
            ...
            _pSettingsModel->setConnectionState(connectionId,
                                                pProjectSettings->general.connectionSettings[idx].bConnectionState);
