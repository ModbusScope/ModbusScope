      ...
      _pDataParserModel(pDataParserModel)
{
    ...
    _pOverlayLabel = new OverlayLabel(tr("No signals configured — click Signals to add one"), _pUi->customPlot);
...
void MainWindow::showRegisterDialog()
{
    if (_pGuiModel->guiState() == GuiState::DATA_LOADED)
    {
        ...
        reply = QMessageBox::question(
          this, "Clear data?",
          "An imported data file is loaded. Do you want to clear the data and start adding signals for a new log?",
    ...
    {
        qCWarning(scopeComm) << "MainWindow: no adapters available — cannot open signals dialog";
        return;
    }
